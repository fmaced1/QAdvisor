...
class Macd:
    ...
    def macd_analisys(stocks_df, signals_of_revert, candles):
        ...
        if len(stocks_df.index) > candles:
            ...
            if (stocks_df['macdh_a'][-candles:].apply(lambda sentence: any(word in sentence for word in signals_of_revert))).any():
